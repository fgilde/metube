...
The complete reference inherited from [upstream MeTube](https://github.com/alexta69/metube), adjusted for this fork's image. Fork-specific settings are documented in [Direct routes](direct-routes) and [Authentication](authentication).

## 🐳 Run using Docker

```bash
docker run -d -p 8081:8081 -v /path/to/downloads:/downloads ghcr.io/fgilde/metube
```
...
* __ADMIN_USERNAME__ / __ADMIN_PASSWORD__: Set both to create the first admin and enable the login screen — see [Authentication](authentication).
* __DIRECT_ROUTES__ / __DIRECT_ROUTES_KEY__: Enable (default `true`) and protect the direct links — see [Direct routes](direct-routes).
